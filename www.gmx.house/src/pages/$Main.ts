...
export default ({ baseRoute = "" }: Website) =>
  component(
    (
      ...
      [requestAccountTradeList, requestAccountTradeListTether]: Behavior<
        IAccountTradeListParamApi,
        IAccountTradeListParamApi
      >,
      [requestLeaderboardTopList, requestLeaderboardTopListTether]: Behavior<
        ILeaderboardRequest,
        ILeaderboardRequest
      >,
      [requestOpenTrades, requestOpenTradesTether]: Behavior<
        IPagePositionParamApi,
        IPagePositionParamApi[]
      >,
      [requestPricefeed, requestPricefeedTether]: Behavior<
        IPricefeedParamApi,
        IPricefeedParamApi
      >,
      [requestLatestPriceMap, requestLatestPriceMapTether]: Behavior<
        IChainParamApi,
        IChainParamApi
      >,
      [competitionCumulativePnl, competitionCumulativePnlTether]: Behavior<
        IQueryCompetitionApi,
        IQueryCompetitionApi
      >,
      [competitionCumulativeRoi, competitionCumulativeRoiTether]: Behavior<
        IQueryCompetitionApi,
        IQueryCompetitionApi
      >,
      [requestTrade, requestTradeTether]: Behavior<
        IIdentifiableEntity,
        IIdentifiableEntity
      >,
      [walletChange, walletChangeTether]: Behavior<
        IEthereumProvider | null,
        IEthereumProvider | null
      >
    ) => {
      ...
      const rootRoute = router.create({
        fragment: baseRoute,
        title: "Community Leaderboard - GMX.house",
        fragmentsChange,
      });
      const chainRoute = rootRoute.create({
        fragment: /avalanche|arbitrum/,
        title: "",
      });
      const leaderboardRoute = chainRoute.create({
        fragment: "leaderboard",
        title: "Leaderboard",
      });
      const accountRoute = chainRoute.create({
        fragment: "account",
        title: "Portfolio",
      });

      // competition
      const competitionCumulativeRoiRoute = chainRoute.create({
        fragment: "top-roi",
        title: "Top ROI - Avalanche Trading Competition",
      });
      const competitionCumulativePnlRoute = chainRoute.create({
        fragment: "top-profit",
        title: "Top PnL - Avalanche Trading Competition",
      });
      ...
      const cardRoute = rootRoute
        ...
        .create({
          fragment: new RegExp(
            `^(${TradeStatus.OPEN}|${TradeStatus.CLOSED}|${TradeStatus.LIQUIDATED})$`
          ),
        })
      ...
      const claimMap = replayLatest(
        map(
          (list) => groupByMap(list, (item) => item.account.toLowerCase()),
          claimListQuery()
        )
      );
      ...
      const defaultWalletProvider: Stream<IEthereumProvider | null> =
        awaitPromises(
          map(async (name) => {
            const provider =
              name === "walletConnect"
                ? wallet.walletConnect
                : await wallet.metamaskQuery;
            if (name && provider) {
              const [mainAccount]: string[] = (await provider.request({
                method: "eth_accounts",
              })) as any;
              if (mainAccount) {
                return provider;
      ...
      const latestPriceMap = replayLatest(
        multicast(
          map(
            (res: IPriceLatestMap) =>
              Object.entries(res).reduce((seed, [key, price]) => {
                const k = key as
                  | ARBITRUM_TRADEABLE_ADDRESS
                  | AVALANCHE_TRADEABLE_ADDRESS;
            ...
            clientApi.requestLatestPriceMap
          )
        )
      );
      ...
      return [
        mergeArray([
          $node(
            ...
            style({
              fontSize: "1.1rem",
              backgroundImage: `radial-gradient(570% 71% at 50% 15vh,${pallete.horizon} 0,${pallete.background} 100%)`,
              alignItems: "center",
              placeContent: "center",
            })
          )(
                ...
                $row(
                  ...
                  $row(layoutSheet.spacingBig, style({ alignItems: "center" }))(
                    ...
                    $anchor(
                      ...
                      style({ padding: "0 4px" }),
                      attr({
                        href: "https://github.com/nissoh/gambit-community",
                      })
                    )(
                      $icon({
                        $content: $github,
            ...
            router.contains(chainRoute)(
              $column(
                ...
                $row(
                  layoutSheet.spacing,
                  style({
                    padding: screenUtils.isDesktopScreen
                      ? "34px 15px"
                      : "18px 12px 0",
                    zIndex: 30,
                    alignItems: "center",
                  ...
                  $MainMenu({
                    ...
                    claimMap,
                    parentRoute: chainRoute,
                    containerOp: style({ padding: "34px, 20px" }),
                  })({
                    routeChange: linkClickTether(),
                    walletChange: walletChangeTether(),
                  }),
                ...
                router.match(leaderboardRoute)(
                  $column(layoutSheet.spacingBig)(
                    ...
                    $Leaderboard({
                      ...
                      openTrades: map(
                        (x: IPageParapApi<ITradeOpen>) => ({
                          ...x,
                          page: x.page.map(fromJson.toTradeJson),
                        }),
                        clientApi.requestOpenTrades
                      ),
                      requestLeaderboardTopList: map(
                        (data: IPageParapApi<IAccountSummary>) => ({
                      ...
                      requestLeaderboardTopList:
                        requestLeaderboardTopListTether(),
                ...
                router.match(competitionCumulativePnlRoute)(
                  $column(
                    ...
                    $CumulativePnl({
                      ...
                      parentRoute: chainRoute,
                      parentStore: rootStore,
                      competitionCumulativePnl: map(
                        (x: IPageParapApi<IAccountLadderSummary>) => ({
                          ...x,
                          page: x.page.map((obj) => ({
                            ...fromJson.accountSummaryJson(obj),
                            pnl: BigInt(obj.pnl),
                          })),
                        }),
                      ...
                      competitionCumulativePnl: competitionCumulativePnlTether(
                        map((page) => {
                          return {
                            ...page,
                            from: COMPETITION_START,
                ...
                router.match(competitionCumulativeRoiRoute)(
                  $column(
                    ...
                    $CompetitionRoi({
                      ...
                      competitionCumulativeRoi:
                        competitionCumulativeRoiTether(),
                      routeChange: linkClickTether(),
                    })
                  )
                ),
                ...
                router.contains(accountRoute)(
                  $Account({
                    ...
                    latestPriceMap,
                    tradeList: map(
                      (res: ITradeOpen[]) => res.map(fromJson.toTradeJson),
                      clientApi.requestAccountTradeList
                    ),
                    settledPosition: clientApi.requestTrade,
                    pricefeed: map(
                      (feed: IPricefeed[]) => feed.map(fromJson.pricefeedJson),
                      clientApi.requestPricefeed
                    ),
                    walletLink,
                    walletStore,
                  })({
                ...
                router.contains(tradeRoute)(
                  $Trade({
                    ...
                    parentStore: rootStore,
                    trade: map(
                      (trade) => (trade ? fromJson.toTradeJson(trade) : null),
                      clientApi.requestTrade
                    ),
          ...
          router.contains(cardRoute)(
            $node(
              ...
              style({
                overflow: "hidden",
                fontWeight: 300,
                backgroundImage: `radial-gradient(100vw 50% at 50% 15vh,${pallete.horizon} 0,${pallete.background} 100%)`,
                alignItems: "center",
                placeContent: "center",
              })
            )(
