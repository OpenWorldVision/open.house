...
export interface ILeaderboard<T extends BaseProvider> {
  ...
  parentStore: <Z, TK extends string = string>(
    key: TK,
    intitialState: Z
  ) => state.BrowserStore<Z, TK>;
}

const topSettledOption = "Top Settled";
const topOpenOption = "Top Open";

export const $Leaderboard = <T extends BaseProvider>(config: ILeaderboard<T>) =>
  component(
    (
      [topPnlTimeframeChange, topPnlTimeframeChangeTether]: Behavior<
      any,
      ILeaderboardRequest["timeInterval"]
      >,

      [routeChange, routeChangeTether]: Behavior<string, string>,
      [tableTopPnlRequest, tableTopPnlRequestTether]: Behavior<number, number>,
      [openPositionsRequest, openPositionsRequestTether]: Behavior<
      number,
      number
      >,
      [
        tableTopSettledSortByChange,
        tableTopSettledsortByChangeTether,
      ]: Behavior<ISortBy<IAccountSummary>, ISortBy<IAccountSummary>>,
      [tableTopOpenSortByChange, tableTopOpenSortByChangeTether]: Behavior<
      ISortBy<ITradeOpen>,
      ISortBy<ITradeOpen>
      >,
      ...
      const [chainLabel] = urlFragments.slice(1) as [
        keyof typeof CHAIN_LABEL_ID
      ];
      const chain = CHAIN_LABEL_ID[chainLabel];

      const $header = $text(style({ fontSize: "1.15em" }));

      const timeFrameStore = config.parentStore<
      ILeaderboardRequest["timeInterval"]
      >("timeframe", intervalInMsMap.HR24);
      const tableTopSettledSortByStore = config.parentStore<
      ISortBy<IAccountSummary>
      >("tableTopSettledSortByStore", {
        name: "realisedPnl",
        direction: "asc",
      });
      const tableTopOpenSortByStore = config.parentStore<ISortBy<ITradeOpen>>(
        "tableTopOpenSortByStore",
        { name: "realisedPnl", direction: "asc" }
      );
      const selectedMobileContent = config.parentStore<string>(
        "selectedMobileContent",
        topSettledOption
      );

      const tableTopSettledSortBy = startWith(
        tableTopSettledSortByStore.state,
        tableTopSettledSortByStore.store(
          tableTopSettledSortByChange,
          map((x) => x)
        )
      );
      const tableTopOpenSortBy = startWith(
        tableTopOpenSortByStore.state,
        tableTopOpenSortByStore.store(
          tableTopOpenSortByChange,
          map((x) => x)
        )
      );
      const filterByTimeFrameState = replayLatest(
        multicast(
          startWith(
            timeFrameStore.state,
            timeFrameStore.store(
              topPnlTimeframeChange,
              map((x) => x)
            )
          )
        )
      );
      const selectedMobileContentState = replayLatest(
        multicast(
          startWith(
            selectedMobileContent.state,
            selectedMobileContent.store(
              selectMobileContent,
              map((x) => x)
            )
          )
      ...
      const tableRequestState = snapshot(
        (
          {
            filterByTimeFrameState: timeInterval,
            tableTopSettledSortBy: sortBy,
          },
          page
        ): IChainParamApi & ILeaderboardRequest => {
        ...
        combineObject({ tableTopSettledSortBy, filterByTimeFrameState }),
        tableTopPnlRequest
      );

      const tableTopOpenState = snapshot(
        (sortBy, page): IOpenTradesParamApi => {
          return {
            offset: page * 20,
            pageSize: 20,
            chain,
      ...
      const openPositions: Stream<TablePageResponse<ITradeOpen>> = map(
        (res) => {
          return {
            page: res.page,
      ...
      const activeTimeframe: StyleCSS = {
      ...
      const $topAccounts = $Table2<IAccountSummary>({
        $container: $card(
          ...
          style({
            padding: screenUtils.isMobileScreen ? "16px 8px" : "20px",
            margin: "0 -12px",
          })
        ),
        ...
        columns: [
          accountTableColumn,
          {
            ...
            $body: map((pos) => {
              return $row(
                $text(
                  `${pos.winTradeCount}/${
                    pos.settledTradeCount - pos.winTradeCount
                  }`
                )
              );
            }),
          ...
          },
          {
            ...
            $body: map((pos) => $row($ProfitLossText(pos.realisedPnl))),
          },
        ],
      })({
      ...
      const $topOpen = $Table2<ITradeOpen>({
        ...
        columns: [
          accountTableColumn,
          {
            $head: $text("Entry"),
            columnOp: O(
              style({ maxWidth: "58px", flexDirection: "column" }),
            ...
            $body: map((pos) => {
              return $Link({
                ...
                url: `/${getChainName(chain).toLowerCase()}/${
                  TOKEN_ADDRESS_TO_SYMBOL[pos.indexToken]
                }/${pos.id}/${pos.timestamp}`,
          ...
          },
          {
            ...
            $body: map((trade) => {
              const positionMarkPrice = map(
                (priceMap) => priceMap[trade.indexToken].value,
        ...
        sortBy: tableTopOpenSortByChangeTether(),
      });

      const $topAccountsFilter = $row(
        style({
          fontSize: "0.85em",
        ...
        $row(layoutSheet.spacing)(
          ...
          $anchor(
            styleBehavior(
              map(
                (tf) => (tf === intervalInMsMap.HR24 ? activeTimeframe : null),
                filterByTimeFrameState
              )
            ),
            topPnlTimeframeChangeTether(
              nodeEvent("click"),
              constant(intervalInMsMap.HR24)
            )
          )($text("24Hour")),
          $anchor(
            styleBehavior(
              map(
                (tf) => (tf === intervalInMsMap.DAY7 ? activeTimeframe : null),
                filterByTimeFrameState
              )
            ),
            topPnlTimeframeChangeTether(
              nodeEvent("click"),
              constant(intervalInMsMap.DAY7)
            )
          )($text("7Day")),
          $anchor(
            styleBehavior(
              map(
                (tf) => (tf === intervalInMsMap.MONTH ? activeTimeframe : null),
                filterByTimeFrameState
              )
            ),
            topPnlTimeframeChangeTether(
              nodeEvent("click"),
              constant(intervalInMsMap.MONTH)
            )
          )($text("1Month"))
        )
      );
              ...
              map((showContent) => {
                return $column(layoutSheet.spacing)(
                  ...
                  $card(
                    layoutSheet.spacingBig,
                    style({
                      padding: screenUtils.isMobileScreen
                        ? "16px 8px"
                        : "20px",
                      margin: "0 -12px",
                    })
