...
import { providerTestnet } from "../rpc";
import { prepareClient } from "./common";

import { TypedDocumentNode } from "@urql/core";
import { cacheMap } from "../utils";
import { getIdentityFromENS } from "../../../common/src/ensIdentity";
...
api.post("/claim-account-twitter", async (req, res) => {
  ...
  const existingClaim = await EM.findOne(Claim, {
    account: { $eq: parsedClaim.account },
  });
...
api.post("/claim-account-ens", async (req, res) => {
  ...
  if (isAddress(account)) {
    try {
      const ensData = await getIdentityFromENS(account, providerTestnet);
      const existingClaim = await EM.findOne(Claim, {
        account: { $eq: account },
      });
...
export const fetchTrades = async <T, R extends IPagePositionParamApi, Z>(
  ...
  const resp = await graphMap[chain](
    doc,
    { ...params, offset },
  ...
  if (nextOffset > 5000) {
    console.warn(
      `query has exceeded 5000 offset at timefram ${intervalInMsMap.DAY7}`
    );
    return list;
  }
...
export const fetchHistoricTrades = async <
  T,
  R extends IPagePositionParamApi & ITimerangeParamApi,
  Z
>(
  doc: TypedDocumentNode<T, R>,
  params: R,
  ...
  if (deltaTime >= intervalInMsMap.DAY7) {
    ...
    const query0 = fetchTrades(
      doc,
      { ...params, to: params.to - splitDelta },
      chain,
      0,
      getList
    );
    const query1 = fetchTrades(
      doc,
      { ...params, from: params.to - splitDelta },
      chain,
      0,
      getList
    );
