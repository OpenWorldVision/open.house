...
import {
  ...
  latestPriceTimelineQuery,
  pricefeed,
  tradeSettledListQuery,
  tradeOpenListQuery,
} from "./queries";
...
export const requestLeaderboardTopList = O(
  map(async (queryParams: ILeaderboardRequest) => {
    ...
    const list = globalCache(cacheKey, cacheLife, async () => {
      ...
      const tradeList = await fetchHistoricTrades(
        tradeSettledListQuery,
        { from, to: timeNow, offset: 0, pageSize: 1000 },
        queryParams.chain,
        0,
        (res) => res.trades
      );
...
export const requestAccountTradeList = O(
  map(async (queryParams: IAccountTradeListParamApi) => {
    const allAccounts = await graphMap[queryParams.chain](
      accountTradeListQuery,
      { ...queryParams, account: queryParams.account.toLowerCase() },
...
export const requestLatestPriceMap = O(
  map(async (queryParams: IChainParamApi): Promise<IPriceLatestMap> => {
    const priceList = await graphMap[queryParams.chain](
      latestPriceTimelineQuery,
      {},
      { requestPolicy: "network-only" }
    );
    const gmap = groupByMap(
      priceList.priceLatests.map(fromJson.priceLatestJson),
      (price) => price.id
    );
    return gmap;
  }),
...
export const lightningLatestPricesMap = replayLatest(
  multicast(
    merge(
      combine(
        (ap, bp) => ({ ...ap, ...bp }),
        requestLatestPriceMap(
          constant({ chain: CHAIN.ARBITRUM }, periodic(15000))
        ),
        requestLatestPriceMap(
          constant({ chain: CHAIN.AVALANCHE }, periodic(15000))
        )
      ),
      combine(
        (ap, bp) => ({ ...ap, ...bp }),
...
export const requestOpenTrades = O(
  snapshot(async (feedMap, queryParams: IOpenTradesParamApi) => {
    const query = globalCache(
      "requestOpenTrades" + queryParams.chain,
      ...
      async () => {
        ...
        return list.map((tradeJson) => {
          ...
          const { delta, deltaPercentage } = calculatePositionDelta(
            marketPrice,
            trade.averagePrice,
            trade.isLong,
            trade
          ...
          return {
            ...trade,
            realisedPnl: delta,
            realisedPnlPercentage: deltaPercentage,
          };
        });
      }
    );
...
export const requestTrade = O(
  map(async (queryParams: IRequestTradeQueryparam) => {
    ...
    const priceFeedQuery = await graphMap[queryParams.chain](
      tradeQuery,
      { id },
      { requestPolicy: "network-only" }
    );
...
export const requestPricefeed = O(
  map(async (queryParams: IPricefeedParamApi) => {
    ...
    const params = {
      tokenAddress,
      interval: "_" + queryParams.interval,
