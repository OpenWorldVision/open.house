...
const run = async () => {
  ...
  app.use((req, res, next) => {
    if (
      ...
      const profilePageMatches = req.originalUrl.match(
        /(\/p\/account\/)(0x[a-fA-F0-9]{40}-(closed-\d+-\d+|liquidated-\d+-\d+|open-\d+-\d+))\/(0x[A-Fa-f0-9]{64})$/i
      );
      ...
      if (profilePageMatches?.length === 5) {
        const [token, tradeType, startDate, endDate] =
          profilePageMatches[2].split("-");
        ...
        const ogHtmlFile = htmlFile
          ...
          .replace(
            /\$OG_IMAGE/g,
            `${process.env.OPENGRAPH_SERVICE}/og-trade-preview?tradeType=${tradeType}&tradeId=${tradeId}&startDate=${startDate}&endDate=${endDate}&token=${token}`
          );
