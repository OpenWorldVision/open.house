...
function wsConnection(
  wss: EventEmitter,
  clientPipeList: Op<WsData, string>[]
): Stream<string> {
  return {
    run(sink, scheduler) {
      ...
      const connectToMessages = chain((ws) => {
        const clientIn = fromCallback<WsData>((cb) =>
          ws.on("message", (data) => cb({ data, ws }))
        );
...
export const helloFrontend = <T extends ILoopMap<T>, OUT>(
  wss: ws.Server | ws,
  inMap: T
) => {
  const entriesInMap: [string, Op<WsData, any>][] = Object.entries(inMap);

  const composedPipes = entriesInMap.map(
    ([topic, op]) =>
      O(
        map((msg: WsData) => {
        ...
        filter((data: ICommunicationMessage<string, any>) => {
          const isMessageValid =
            typeof data === "object" && "body" in data && "topic" in data;
