...
import { IAccountLadderSummary } from "../../common/src/types";
...
export const cacheMap =
  ...
  async <T>(
    ...
    if (cacheEntry && !cacheMap[key].lifespanFn()) {
      ...
      const lifespanFn =
        cacheMap[key]?.lifespanFn ?? timespanPassedSinceInvoke(lifespan);
      lifespanFn();
...
export function toAccountCompetitionSummary(
  list: ITrade[],
  priceMap: { [k: string]: IPricefeed },
  endDate: number
): IAccountLadderSummary[] {
  ...
  return tradeListEntries.reduce((seed, [account, tradeList]) => {
    ...
    const { maxUsedCollateral } = adjustmentsDuringTimerange.reduce(
      (seed, next) => {
        ...
        const nextUsedCollateral = Object.values(seed.positions).reduce(
          (s, n) => s + n,
          0n
        );
    ...
    const summary = sortedTradeList.reduce(
      (seed, next): IAccountLadderSummary => {
        ...
        const tradeMaxCollateral = filteredUpdates.reduce(
        ...
        const maxCollateral =
          usedMinProfit > maxUsedCollateral ? usedMinProfit : maxUsedCollateral;

        const roi = div(
          pnl,
          maxCollateral > MIN_OF_MAX_COLLATERAL
            ? maxCollateral
            : MIN_OF_MAX_COLLATERAL
        );
        ...
        const cumulativeLeverage =
          seed.cumulativeLeverage + div(lastUpdate.size, maxCollateral);
        ...
        return {
          ...
          realisedPnlPercentage:
            seed.realisedPnlPercentage + next.realisedPnlPercentage,
